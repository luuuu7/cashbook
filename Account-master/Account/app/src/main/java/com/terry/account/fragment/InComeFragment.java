 ...
 * @author sjy
 */
public class InComeFragment extends BaseSwitchFragment implements
    ...
    private List<TypeBean> mAllList = new ArrayList<>();

    private static final String mComeType = "income";

    public static InComeFragment getInstance() {
        if (instance == null) instance = new InComeFragment();
        return instance;
    }
    ...
    private void showAdd(){
        new MaterialDialog.Builder(mAct).title(R.string.add_income)
                .positiveText(android.R.string.ok)
                .inputRange(2, 10)
                .input(R.string.add_income_hint, R.string.input_empty, false, new MaterialDialog.InputCallback() {
                    @Override
                    public void onInput(@NonNull MaterialDialog dialog, CharSequence input) {
                        ...
                        if(isExist){
                            SingleToast.showToast(mAct,"该收入类型已存在，请重新输入!",2000);
                            return;
                        }
                        ...
                        SingleToast.showToast(getContext(),"已添加资产",2000);
                        doRefresh();
                    }
                })
