...
import com.terry.account.activity.ColumnChartActivity;
 ...
 * @author sjy
 */
public class YearFragment extends BaseSwitchFragment implements AccountAdapter.OnAccountClick
    ...
    private static YearFragment instance;
    ...
    public static YearFragment getInstance() {
        if (instance == null) instance = new YearFragment();
        return instance;
    }
    ...
    private void initFloatBtn() {
        ...
        fbChoseDay.setLabelText(getString(R.string.fl_menu_choseyear));
        ...
        FloatingActionButton fbLineChart = new FloatingActionButton(mAct);
        fbLineChart.setButtonSize(FloatingActionButton.SIZE_MINI);
        fbLineChart.setLabelText(getString(R.string.fl_menu_line_chart));
        fbLineChart.setImageDrawable(new IconicsDrawable(mAct, GoogleMaterial.Icon.gmd_line_weight).color(Color.WHITE).actionBar());
        fbLineChart.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                //查看图表
                mFabActMenu.close(false);
                if(mAllList.isEmpty()){
                    ...
                    mApp.mAccBeanChartList = mAllList;
                    Intent itChar = new Intent(mAct,ColumnChartActivity.class);
                    itChar.putExtra("title",qryTime+"柱状图表");
                    startActivity(itChar);
                }
            }
        });
        mFabActMenu.addMenuButton(fbLineChart);
    ...
    private void doRefresh() {
        mSwipeRefreshLayout.setRefreshing(true);
        String minTime = qryTime+"-01-01";
        String maxTime = String.valueOf(Integer.parseInt(qryTime)+1)+"-12-31";
        ...
        mAdapter = new AccountAdapter(mAct, mAllList, YearFragment.this);
...
//        mTxTotal.setText("总计金额:  " + resBig.floatValue());
         ...
         */
        Float max = Float.valueOf(mApp.mSelfYearMax);
        if (max != -1 && max < -resBig.floatValue()) {
            new MaterialDialog.Builder(mAct).title("本年度支出已超支")
                    .content("本年度已超支" + (new DecimalFormat("0.00").format(-resBig.floatValue() - max)) + "元，请注意~")
    ...
    final DatePickerDialog.OnDateSetListener OnDialogDateSetListener = new DatePickerDialog.OnDateSetListener() {
